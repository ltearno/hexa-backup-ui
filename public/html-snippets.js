...
function itemToHtml(f, currentName) {
    if (f.mimeType == 'application/directory')
        ...
        return `<div class="onclick"><i><a href='${Locations.getReferenceUrl(f.name)}'>${f.name}</a> ...</i></div>`;
    else if (f.mimeType == 'application/playlist')
        return `<div class="onclick"><i><a href='${Locations.getPlaylistUrl(f.name)}'>${f.name}</a> ...</i></div>`;
